...
namespace CRUD1._7.Controllers
{
    public class TYPE_TRAMOController : Controller
    {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            var datacarril = db.TYPE_CARRIL.ToList();
            TYPE_CARRIL cARRIL = new TYPE_CARRIL();
            foreach (var item in dataplaza)
            {
                if (item.idTramo == tYPE_TRAMO.idenTramo)
                {
                    foreach (var item2 in datacarril)
                    {
                        if (item2.idPlaza == item.idenPlaza)
                        {
                            db.TYPE_CARRIL.Remove(item2);
                        }
                    }
        ...
        {
            TYPE_PLAZA pLAZA = db.TYPE_PLAZA.Find(id);
            TYPE_CARRIL carriles = new TYPE_CARRIL();
            var data = db.TYPE_CARRIL.ToList();
            foreach (var item in data)
            {
                if (item.idPlaza == pLAZA.idenPlaza)
                {
        ...
        public ActionResult VerCarriles(int id)
        {
            ...
            if (carrilesdeplaza != null && carrilesdeplaza.Count != 0)
            {
                ...
                selectTramos.Add(new tramos
                {
                    idenTramo = carrilesdeplaza.FirstOrDefault().pla.idTramo.Value
                });
                model.Plazas = selectPlazas;
                model.Tramos = selectTramos;
                model.Carriles = selectCarriles;
