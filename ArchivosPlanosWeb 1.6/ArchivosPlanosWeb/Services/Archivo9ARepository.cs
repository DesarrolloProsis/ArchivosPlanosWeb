    ...
    public class Archivo9ARepository
    {
        ...
        public void eventos_detectados_y_marcados_en_el_ECT(string Str_Turno_block, DateTime FechaInicio, string IdPlazaCobro, string CabeceraTag, string Tramo)
        {
            ...
            try
            {
                ...
                //DATE_DEBUT_POSTE
                StrQuerys = "SELECT DATE_TRANSACTION, VOIE,  EVENT_NUMBER, FOLIO_ECT, Version_Tarif, ID_PAIEMENT, INDICE_SUITE," +
                ...
                if (MtGlb.QueryDataSet(StrQuerys, "TRANSACTION"))
                {
                    ...
                    foreach (DataRow item in MtGlb.Ds.Tables["TRANSACTION"].Rows)
                    {
                        ...
                        if (item["INDICE_SUITE"].ToString() == "9")
                        {
                            ...
                            if (MtGlb.QueryDataSet3(StrQuerys, "TRANSACTION"))
                            {
                                ...
                                if (!DBNull.Value.Equals(item))
                                {
                                    ...
                                    if (Convert.ToInt32(item["ID_PAIEMENT"]) == 15)
                                    {
                                        ...
                                        Tag_iag = Tag_iag.Substring(0, 17).Trim();
